...
class Preprocess_data:
	...
	def print_vocab(self):
		for k in sorted(self.vocabulary.keys()):
			print k,len(self.vocabulary[k])

	def play_word(self,word=''):
		if word=='':
			for k in self.vocabulary.keys():
				for i in xrange(len(self.vocabulary[k])):
					wavfile.write('./vocab/'+k+str(i)+'.wav',8000,self.vocabulary[k][i])
		else:
			if self.vocabulary.has_key(word)==0:
				print "ERROR:word not in vocabulary"
				return

			for i in xrange(len(self.vocabulary[word])):
				wavfile.write(word+str(i)+'.wav',8000,self.vocabulary[word][i])
...
if __name__=="__main__":
	...
	data_inputs=[]
	data_outs=[]
	for w in  tts_obj.vocabulary.keys()  :
		embs = tts_obj.generate_embeddings(w,2000,1000)
		data_inputs.append(embs)
		data_outs.append( [ 1 if w==nw else 0 for nw in sorted(narated_obj.vocabulary.keys()) ] )

	inputs=np.random.rand(len(data_inputs),len(data_inputs[0]),2000)
	outputs = np.random.rand( len(data_outs), len(data_outs[0] ) )
	print inputs.shape,outputs.shape

	batch_size=500
	n_batches = len(data_inputs)/batch_size

	for batch in xrange(n_batches):
		inputs=np.random.rand(batch_size,len(data_inputs[0]),2000)
		outputs = np.random.rand( batch_size, len(data_outs[0] ) )
		print inputs.shape,outputs.shape
		...
		with open('datafile_'+str(batch), 'wb') as f:
			pickle.dump( [ inputs,outputs ], f, pickle.HIGHEST_PROTOCOL)
